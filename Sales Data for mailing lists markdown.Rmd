...
```{r compute statistics for each city, include=TRUE}
# Stats for SF
...
Metro_sales %>%  
  ggplot() +
  geom_horizon(aes(ContractDate, 
                   GrossAmount), origin = 'min', horizonscale = 4) +
  ...
  ggtitle('Sales by Zipcode in the Metro area') +
  xlab('Date')
```

```{r test environment, include=TRUE}
scatterplot3d(x = Metro_sales$Latitude, y = Metro_sales$Longitude, z = Metro_sales$GrossAmount)
```

```{r citation, include=TRUE}
citation("ggmap")
sessionInfo()
```
