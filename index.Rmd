...
Sales_matrix <- data.frame(Sales$ContractDate, Sales$GrossAmount, Sales$productid)

Sales_matrix <- Sales_matrix[Sales_matrix$Sales.ContractDate > "2000-01-01" & Sales_matrix$Sales.ContractDate < "2023-12-31",]

# Seperate all product lines
Sales_series_patio_cover = subset(Sales_matrix, Sales_matrix$Sales.productid == "PC")
Sales_series_sunroom = subset(Sales_matrix, Sales_matrix$Sales.productid == "Sun")
Sales_series_bath = subset(Sales_matrix, Sales_matrix$Sales.productid == "Bath")
Sales_series_kitchen = subset(Sales_matrix, Sales_matrix$Sales.productid == "Kitchen")
Sales_series_window = subset(Sales_matrix, Sales_matrix$Sales.productid == "Win")
...
combined_series <- left_join(date_series, Sales_no_outlier, by = join_by("Date" == "Sales.ContractDate"))
...
Sales_series <- data.frame(Date = combined_series$Date,
                           ...
                           Day = cut(combined_series$Date,
                                      breaks = "day",
                                      labels = FALSE,
                                      start.on.monday = TRUE) %% 366,
                           GrossAmount = combined_series$Sales.GrossAmount)

# Plot the different time series for each sales category
p2 = ggplot(Sales_series, aes(x = Day, y = Year, fill = GrossAmount)) +
  geom_raster() +
